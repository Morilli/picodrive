...
{
	PicoIn.autoRgnOrder = regionAutoOrder;
	PicoIn.regionOverride = regionOverride;

	p32x_bios_g = TryLoadBios("32x.g");
	p32x_bios_m = TryLoadBios("32x.m");
	p32x_bios_s = TryLoadBios("32x.s");

	PicoIn.opt = POPT_EN_FM | POPT_EN_PSG | POPT_EN_Z80 | POPT_EN_STEREO | POPT_ACC_SPRITES | POPT_DIS_32C_BORDER | POPT_EN_MCD_PCM | POPT_EN_MCD_CDDA | POPT_EN_MCD_GFX | POPT_EN_32X | POPT_EN_PWM | POPT_DIS_IDLE_DET;
	...
	PicoIn.sndOut = sound_buffer;
	PicoIn.writeSound = SoundCallback;
...
ECL_EXPORT void FrameAdvance(MyFrameInfo *f)
{
	current_frame = f;
	PicoInputWasRead = 0;
	PicoIn.pad[0] = f->Buttons & 0xfff;
	PicoIn.pad[1] = f->Buttons >> 12 & 0xfff;
...
ECL_EXPORT void GetMemoryAreas(MemoryArea *m)
{
	...
	m[1].Data = PicoMem.vram;
	m[1].Name = "VRAM";
	m[1].Size = 0x10000;
	m[1].Flags = MEMORYAREA_FLAGS_WRITABLE | MEMORYAREA_FLAGS_WORDSIZE2 | MEMORYAREA_FLAGS_YUGEENDIAN | MEMORYAREA_FLAGS_SWAPPED;

	m[2].Data = PicoMem.zram;
	m[2].Name = "Z80 RAM";
	m[2].Size = 0x2000;
	m[2].Flags = MEMORYAREA_FLAGS_WRITABLE | MEMORYAREA_FLAGS_WORDSIZE1;

	m[3].Data = PicoMem.cram;
	m[3].Name = "CRAM";
	m[3].Size = 0x80;
	m[3].Flags = MEMORYAREA_FLAGS_WRITABLE | MEMORYAREA_FLAGS_WORDSIZE2 | MEMORYAREA_FLAGS_YUGEENDIAN | MEMORYAREA_FLAGS_SWAPPED;

	m[4].Data = PicoMem.vsram;
	m[4].Name = "VSRAM";
	m[4].Size = 0x80;
	m[4].Flags = MEMORYAREA_FLAGS_WRITABLE | MEMORYAREA_FLAGS_WORDSIZE2 | MEMORYAREA_FLAGS_YUGEENDIAN | MEMORYAREA_FLAGS_SWAPPED;
	...
	{
		m[8].Data = Pico.sv.data;
		m[8].Name = "SRAM";
		m[8].Size = Pico.sv.size;
